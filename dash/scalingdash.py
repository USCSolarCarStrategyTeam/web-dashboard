...
class Dashboard(QWidget):
    ...
    def adjustScaling(self):
        ...
        if (w != prevW or h != prevH):
            ...
            # s = self.currSpeed
            # self.currSpeed.resize(s.width() * w / prevW, s.height() * h / prevH)
            # s = self.currRange
            # self.currRange.resize(s.width() * w / prevW, s.height() * h / prevH)
            # s = self.currBatt
            # self.currBatt.resize(s.width() * w / prevW, s.height() * h / prevH)
            # s = self.currTemp
            # self.currTemp.resize(s.width() * w / prevW, s.height() * h / prevH)
            # s = self.currCurrent
            # self.currCurrent.resize(s.width() * w / prevW, s.height() * h / prevH)
            # s = self.currPower
            # self.currPower.resize(s.width() * w / prevW, s.height() * h / prevH)

            # scaleFactor = .5    #THIS IS SOME EXPONENTIAL SHIT HELP!!
            # valueFontS = QFont("Arial", 50 * (w * h) / (800 * 480) * scaleFactor)
            # valueFontM = QFont("Arial", 100 * (w * h) / (800 * 480) * scaleFactor)
            # valueFontL = QFont("Arial", 150 * (w * h) / (800 * 480) * scaleFactor)
            # self.currSpeed.setFont(valueFontM)
            # self.currRange.setFont(valueFontM)
            # self.currBatt.setFont(valueFontL)
            # self.currTemp.setFont(valueFontS)
            ...
            # s = self.lbl1
            # self.lbl1.resize(s.width() * w / prevW, s.height() * h / prevH)
            # s = self.lbl2
            # self.lbl2.resize(s.width() * w / prevW, s.height() * h / prevH)
            # s = self.lbl3
            # self.lbl3.resize(s.width() * w / prevW, s.height() * h / prevH)
            # s = self.lbl4
            # self.lbl4.resize(s.width() * w / prevW, s.height() * h / prevH)
            # s = self.lbl5
            # self.lbl5.resize(s.width() * w / prevW, s.height() * h / prevH)
            # s = self.lbl6
            # self.lbl6.resize(s.width() * w / prevW, s.height() * h / prevH)
            
            # scaleFactor = .5    #THIS IS SOME EXPONENTIAL SHIT HELP!!
            # fontSize = 20 * (w * h) / (800 * 480)
            # textFont = QFont("Arial", fontSize)
            # self.lbl1.setFont(textFont)
            # self.lbl2.setFont(textFont)
            # self.lbl3.setFont(textFont)
            # self.lbl4.setFont(textFont)
            ...
            # s = self.speedUnit
            # self.speedUnit.resize(s.width() * w / prevW, s.height() * h / prevH)
            # s = self.rangeUnit
            # self.rangeUnit.resize(s.width() * w / prevW, s.height() * h / prevH)
            # s = self.battUnit
            # self.battUnit.resize(s.width() * w / prevW, s.height() * h / prevH)
            # s = self.tempUnit
            # self.tempUnit.resize(s.width() * w / prevW, s.height() * h / prevH)
            # s = self.currentUnit
            # self.currentUnit.resize(s.width() * w / prevW, s.height() * h / prevH)
            # s = self.powerUnit
            # self.powerUnit.resize(s.width() * w / prevW, s.height() * h / prevH)
            
            #BATTERY spacing & sizing       #UPDATE PROPERLY PLS
            self.batteryImage.move(300 * w / 800, 115 * h / 480)
            
            # s = self.batteryImage
            # self.batteryImage.resize(s.width() * (w / prevW), s.height() * (h / prevH))

            # battMap = self.batteryImage.pixmap()
            # self.battScale = 96 * math.sqrt((w * h) / (800.0 * 480.0)) #something to do with scaling exact pixels
            # self.battScale = 96 * (w/800.0)*(h/480.0)
            # battMap = battMap.scaledToHeight(self.battScale)
            # self.batteryImage.setPixmap(battMap)
